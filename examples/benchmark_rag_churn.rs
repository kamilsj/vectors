//! Compare local RAG retrieval across cold, warm, and unrelated-write phases.
//! Run: cargo run --release --example benchmark_rag_churn -- 64 16 128 20
...
use vectors::{
    ...
    GraphRagSelection, GraphRelationshipRequest, Vector,
};
...
fn main() {
    ...
    let from_chunk = "10:document-0:0".to_owned();
    let to_chunk = if per_doc > 1 {
        "10:document-0:1"
    } else {
        "10:document-1:0"
    }
    .to_owned();
    let upsert_relationship = || {
        database
            .graph_upsert_relationship(GraphRelationshipRequest {
                collection: "bench".into(),
                expected_revision: database.revision().unwrap(),
                from_chunk: from_chunk.clone(),
                to_chunk: to_chunk.clone(),
                kind: "adjacent".into(),
                weight: 1.0,
            })
            .unwrap();
    };
    // Ensure the measured relationship upsert preserves graph content too.
    upsert_relationship();
    ...
    for iteration in 0..repetitions {
        ...
        database
            .execute(&format!(
                "UPDATE benchmark_revision_marker SET value = {} WHERE id = 1",
                iteration + 1
            ))
            .unwrap();
        let (unrelated, elapsed) = retrieve(&database, &query);
        let actual = canonical(&unrelated);
        assert_eq!(expected, actual);
        phases[2].record(elapsed, &unrelated, &actual);
        upsert_relationship();
        let (relationship, elapsed) = retrieve(&database, &query);
        let actual = canonical(&relationship);
        assert_eq!(expected, actual);
        phases[3].record(elapsed, &relationship, &actual);
    ...
    println!("{}", serde_json::to_string_pretty(&json!({
        ...
        "phases": {"cold_after_chunk_update": phases[0].report(), "warm_repeat": phases[1].report(),
            "after_unrelated_table_update": phases[2].report(), "after_relationship_upsert": phases[3].report()},
        "identical_results_across_phases": true, "provider_calls": 0,
        "percentile_method": "Sorted samples, index ceil((n - 1) * p), zero-based.",
        "scope": "Local hybrid RAG candidate retrieval and final MMR/context selection. Excludes all writes, fixture setup, provider/network time, HTTP handling and result serialization. Zero graph-expansion hops isolate cache behavior and unchanged ranking."
    })).unwrap());
}
